  ...
  const localTools: any[] = [
    {
      ...
      handler: async ({ fileId, offset = 0, limit = 5000 }: any) => {
        ...
        console.log(`[sidecar] agi_read_file: id=${fileId} offset=${start} length=${slice.length}`)
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (Array.isArray(fileSummaries) && fileSummaries.length > 0) {
      const joined = String(fileSummaries.join('\n- '))
      enhancedSystemPrompt = `${enhancedSystemPrompt}\n\nUploaded Files (summaries only):\n- ${joined}\n\nYou have tools: agi_list_files, agi_read_file. Use them to inspect files on demand instead of relying on summaries.`
    }
...
app.get('/api/tools', async (_req, res) => {
  try {
    res.json({ 
      tools: [
        { name: 'agi_read_file', description: 'Read extracted text content of an uploaded file by id' }
      ]
    })
  } catch (error) {
